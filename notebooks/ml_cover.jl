...
# ╔═╡ 55dbfc50-1c04-11ec-00bb-3138e481ba79
using Colors

# ╔═╡ db849ca7-9c1a-4145-a94a-66b200536046
using ColorSchemes

# ╔═╡ 463e5c4a-ebd7-4b7b-9681-c2ba664f50cf
using Distributions

# ╔═╡ b7ecaffd-95f0-404f-982c-0490e6e02538
begin
    using Convex
    using Plots
    using PyPlot
...
# ╔═╡ 3bb2c71e-fb70-425d-86c4-6839c181d1f1
w = 19.563

# ╔═╡ 2c40185e-e325-4260-a2f7-215f757fc590
h = 10.25
...
begin
    Random.seed!(0)
    𝒟 = []
    push!(𝒟, rand(Normal(0.2, 0.15), 100)) # rate_fail
    push!(𝒟, rand(Normal(0.18, 0.02), 100)) # rate_not_fail
    push!(𝒟, rand(Normal(3.5, 0.1), 100)) # dist_fail
    push!(𝒟, rand(Normal(6, 0.7), 100)) # dist_not_fail
end

# ╔═╡ 69fe17b8-9824-417c-9770-22728fe58c3e
global FCOLOR = "#d62728"

# ╔═╡ 5fbf5aa9-8920-4df3-abf3-5a79bfa55d55
global NFCOLOR = "#2ca02c"

# ╔═╡ ba7d69f0-d2d8-4745-aaa8-659942c6555e
...
# ╔═╡ dea713c9-0f58-467e-a3c0-970216eb2da9
...
# ╔═╡ 1cc80214-3576-4c1b-a516-08361e0c2510
function plot_multivariate_distance_and_rate(𝒟; gda=true, gdatype=:qda, share_cov=false, k=1, svm=false, return_predict=false, boundary=:gradient, subplots=false, figsize=[6.4, 4.8], show_legend=false)
    pp = Plots.PyPlot
    ...
    if gda
        ...
        dbY = range(min(minimum(d_fail), minimum(d_nonfail)) - 2, stop=max(maximum(d_fail), maximum(d_nonfail)), length=1000)
    ...
    # pp.xlabel("rate")
    # pp.ylabel("distance")
    pp.tick_params(bottom=false, labelbottom=false, left=false, labelleft=false)
    ...
    pp.savefig("ml_cover.svg", format="svg")
    fig = pp.gcf()

    return return_predict ? (fig, predict, mv_fail, svm_classify) : fig
end

# ╔═╡ 36229261-46f7-4f56-93c5-d3979bb11aad
plot_multivariate_distance_and_rate(𝒟, figsize=[w,h] ./ 2)
...
git-tree-sha1 = "80bc67ed8b7df15595e7a38816b41e76a821a287"
